...
const CategoryList: React.FC<Props> = ({ game, category }) => {
  ...
  // Helper to get the inner background color for the image frame
  const getFrameBgColor = (item: any) => {
    // For Wuthering Waves Echoes, use class-based colors if available
    if (game === 'wuthering' && category === 'echoes' && item.class) {
       if (item.class === 'Calamity') return 'bg-red-950';
       if (item.class === 'Overlord') return 'bg-red-900/40'; // Slightly lighter red
       if (item.class === 'Elite') return 'bg-purple-900/40';
       if (item.class === 'Common') return 'bg-gray-800';
    }

    // Fallback to Rarity for everything else
    if (item.rarity === 5) return 'bg-yellow-700/20'; // Gold tint
    if (item.rarity === 4) return 'bg-purple-800/20'; // Purple tint
    if (item.rarity === 3) return 'bg-blue-800/20';   // Blue tint
    if (item.rarity === 2) return 'bg-green-800/20';  // Green tint
    
    return 'bg-gray-900'; // Default dark
  };
    ...
    <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-6">
      {items.map((item) => (
        ...
        >
           ...
           <div className={`aspect-square overflow-hidden relative ${getFrameBgColor(item)} transition-colors duration-300`}>
             <img 
                src={item.image} 
                alt={item.name} 
                onError={handleImageError}
                className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110 opacity-90 group-hover:opacity-100 mix-blend-normal" 
             />
             {/* Gradient overlay to blend bottom into card body */}
  ...
  if (category === 'quests') {
      ...
      if (game === 'genshin') {
          ...
          groups = {
              ...
              'Side Quest': data.filter(q => q.category === 'Side Quest'),
              'Exploration Quest': data.filter(q => q.category === 'Exploration Quest')
          };
      }
